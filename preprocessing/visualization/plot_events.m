...
%         legend([a1'],{'\omega_z'});
        ...
        sgtitle(sprintf('%s %s %s Mix Plot', obs.obs_id, obs.traj_type, obs.motion_type ), ...
            'Interpreter','none');
    ...
    %     legend([a1, a2], {'F_{sum} v', 'F_{str} v'})
        ...
        set(hL, 'Position',[0.5 0.03 0.01 0.01],'Units','normalized')        
        sgtitle(sprintf('%s %s %s Angles', obs.obs_id, obs.traj_type, obs.motion_type ), ...
            'Interpreter','none');
...
%         ylim([-20,20]);
        ...
        subplot(srows, scols, 7);
        a1 = plot(tang, accel(:,1:2)); hold on; grid on; 
%         legend([a1'],{'\omega_z'});
        xlim([0, tmax]);
        ylim([-5, 5]);
...
%         legend([a1',a2'],{'\tau_{1z}', '\tau_{2z}'});
        ...
        subplot(srows, scols, 8);
        plot(pos, fstr(:,1)); grid on;
        subtitle('Force-Displacement');
        xlim([0, max(pos)]);
%         ymax = max(20, max(abs(orient)));
%         ylim([-ymax, ymax]);
        xlabel('time, sec');
        plot_vertical_lines(pos_win_locs);
        hold off;
        xlabel('time, sec');
...
function pos_win_locs = get_pos_win_locs(pos, tang, win_locs)
    
    pos_win_locs = zeros(size(win_locs));
    
    for ind1 = 1:size(win_locs,1)
        for ind2 = 1:size(win_locs,2)
            [~,idx] = min(abs(tang-win_locs(ind1,ind2)));
            pos_win_locs(ind1,ind2) = pos(idx);
        end
    end
